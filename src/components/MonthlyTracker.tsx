...
export default function MonthlyTracker({
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-[#A8C686]">Monthly Tracker</h2>
        <div className="flex items-center gap-2">
          <select
            value={month}
            onChange={(e) => setMonth(Number(e.target.value))}
            className="border border-[#A8C686]/50 rounded-lg px-3 py-2 text-sm bg-white text-[#49596B] focus:ring-2 focus:ring-[#A8C686]/50"
          >
            {monthNames.map((m, i) => (
      ...
      {/* Table */}
      <div className="overflow-x-auto rounded-lg border border-[#A8C686]/40">
        <div className="max-h-[500px] overflow-y-scroll">
          <table className="table-fixed w-full text-sm border-collapse">
            <thead className="bg-[#A8C686] sticky top-0 z-10">
              <tr> 
  ...
  {Array.from({ length: daysInMonth }, (_, i) => {
    ...
    return (
      <tr
        key={day}
        className="bg-white/60 border-b border-[#A8C686]/30 last:border-none"
      >
        ...
        {habits.map((h) => (
          <td key={h.id} className="text-center">
            <input
              ...
              className="h-4 w-4 accent-[#A8C686] cursor-pointer"
            />
          </td>
        ))}
        <td className="px-3 py-3 text-center w-20">
          <div className="relative inline-flex items-center justify-center">
            <svg className="w-10 h-10">
              ...
              />
              <circle
                className="text-[#A8C686] transition-[stroke-dashoffset] duration-500 ease-out"
                strokeWidth="3"
                strokeLinecap="round"
                stroke="currentColor"
                fill="transparent"
